...
#endif
  master_printf("Running meep version 1.2\n" );
  master_printf("Edited by ACTT version 0.2.2\n" );
  t_start = wall_time();
}
...
void broadcast(int from, complex<float> *data, int size) {
#ifdef HAVE_MPI
  if (size == 0) return;
  MPI_Bcast(data, 2*size, MPI_FLOAT, from, mycomm);
#else
  UNUSED(from);
  UNUSED(data);
  UNUSED(size);
